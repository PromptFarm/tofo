...
**Fixed (2026-08-07):** `next build`'s page-data-collection phase runs several worker processes in parallel, each independently opening the SQLite DB (`src/lib/sqlite/db.ts`) — against a database file that doesn't exist yet, they raced to run the schema-creation `CREATE TABLE` statements and threw `SQLITE_BUSY` ("database is locked"). This was invisible until CI actually ran `next build` for the first time (previously only the test suite ran in CI). Fixed two ways: `PRAGMA busy_timeout` is now set before `PRAGMA journal_mode = WAL` on every connection open (order matters — `journal_mode` itself touches the file and can lose the race before `busy_timeout` takes effect if set after), and a `prebuild` script (`scripts/warm-db.ts`) creates the DB file and schema once, single-process, before `next build`'s workers start, so there's nothing left to race over. `scripts/build-desktop.sh` (the actual path the release workflow uses) calls `next build` directly rather than through `pnpm run build`, bypassing the `prebuild` hook — it needed the same `warm-db.ts` call added explicitly.

**Fixed (2026-08-07):** fixing the above surfaced a second race in the same area — `syncAppUser()` (`src/lib/db-client.ts`) used a SELECT-then-INSERT to seed the local desktop user, which isn't safe against the same parallel `next build` workers: multiple processes could see "no row yet" and all try to INSERT, and the losers hit a UNIQUE constraint on `User.email`. Replaced with `INSERT ... ON CONFLICT(id) DO UPDATE`, which SQLite resolves atomically regardless of how many callers race it.

**Fixed (2026-08-07):** the desktop shell had two related issues found while investigating a reported "TOFO hangs for up to 30s on launch." First, `.setup()` runs on Tauri's main UI thread, and extracting the bundled ~400MB archive plus polling for the server's TCP port to open (up to 30s) both ran inline there, synchronously — the placeholder window couldn't repaint during either step and looked frozen rather than loading. Second, nothing ever killed the spawned Node child process on quit (`Child` isn't killed on drop, and the tray "Quit" handler's `app.exit(0)` tears the process down without dropping managed state), so the server could survive as an orphan holding the SQLite lock and port 3100, breaking the next launch until killed manually. Fixed by moving the whole startup sequence to a background thread (only the final window mutation is dispatched back via `run_on_main_thread`) and adding a `RunEvent::Exit` handler that kills the managed child. OS-level force-quit still bypasses the latter — would need Windows Job Objects to close that gap, not done here.
